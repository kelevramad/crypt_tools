...
def mock_getpass(monkeypatch):
    ...
    monkeypatch.setattr(crypt_tools, 'getpass_verify_with_strength', lambda prompt1="Enter Password: ", prompt2="Verify Password: ": 'cli_pass')
...
def test_import_error_missing_deps(monkeypatch, capsys):
    ...
    with pytest.raises(SystemExit) as excinfo:
        runpy.run_path(os.path.join(os.path.dirname(crypt_tools.__file__), "crypt_tools.py"), run_name="__main__")
    assert excinfo.value.code == 1
    captured = capsys.readouterr()
...
def test_password_strength_indicator_and_types():
    ...
    assert "Lower" in types_strong and "Upper" in types_strong and "Number" in types_strong and "Symbol" in types_strong
    assert "Lower" in types_weak and "Upper" in types_weak and "Number" in types_weak and "Symbol" in types_weak
...
def test_inspect_file_rejects_plaintext_file(engine, tmp_path):
    ...
    with pytest.raises(ValueError, match="Only CT02 encrypted files can be inspected reliably"):
...
def test_recursive_encrypt_fail_count(monkeypatch, tmp_path):
    password = "pw"
    (tmp_path / "a.txt").write_text("a")
    monkeypatch.setattr(crypt_tools.CryptoEngine, "encrypt_file", lambda self, i, o, p, compress=False: False)
...
def test_recursive_decrypt_no_ext_file(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(crypt_tools.CryptoEngine, "decrypt_file", lambda self, i, o, p, compress=False: False)
