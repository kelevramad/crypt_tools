...
class RecoveryKeyUtils:
	...
	def write_recovery_key(path: str, recovery_key: bytes) -> bool:
		...
		try:
			b64_key = base64.urlsafe_b64encode(recovery_key).decode('ascii').rstrip('=')
			with open(path, 'w', encoding='utf-8', newline='\n') as f:
				f.write(f'{b64_key}\n')
			ConsoleLogger.show('success', f'Recovery key file generated: {path}', icon='🔑')
			ConsoleLogger.show(
				'info',
				f'Key size: {Config.RECOVERY_KEY_SIZE} bytes ({Config.RECOVERY_KEY_SIZE * 8} bits)',
			)
			ConsoleLogger.show(
				'warning',
				'Store recovery key securely - it can decrypt your files without a password!',
				icon='🔐',
			)
			return True
		except Exception as e:
	...
	def read_recovery_key(path: str) -> Optional[bytes]:
		...
		try:
			...
			ConsoleLogger.show('debug', f'Read recovery key: {path} ({len(key_data)} bytes)')
			return key_data
		except Exception as e:
...
class CryptoEngine:
	...
	def encrypt_data(
		...
		if use_recovery:
			...
			recovery_blob = RecoveryKeyUtils.encrypt_derived_key(key, recovery_key)
		...
		if recovery_blob:
			...
			result += len(recovery_blob).to_bytes(2, byteorder='big') + recovery_blob
	...
	def decrypt_data(
		...
		try:
			...
			if metadata['is_legacy']:
				...
				if use_recovery:
					...
					recovery_len = int.from_bytes(
						enc_data[recovery_len_pos : recovery_len_pos + 2], byteorder='big'
					)
					recovery_blob = enc_data[
						recovery_len_pos + 2 : recovery_len_pos + 2 + recovery_len
					]
					ciphertext_offset = 2 + recovery_len
					ConsoleLogger.show('debug', f'Read recovery blob ({recovery_len} bytes)')

				ciphertext = enc_data[nonce_end + ciphertext_offset : -metadata['tag_len']]
			...
			if recovery_key_data and use_recovery and recovery_blob:
				ConsoleLogger.show('info', 'Attempting decryption with recovery key...', icon='🔑')
				key = RecoveryKeyUtils.decrypt_derived_key(recovery_blob, recovery_key_data)
				if key:
					ConsoleLogger.show(
		...
		try:
			...
			with open(input_path, 'rb') as fin:
				...
				if use_recovery:
					...
					recovery_len = int.from_bytes(recovery_len_bytes, byteorder='big')
...
def main(argv=None):
	...
	if args.recovery_key:
		if args.decrypt:
			...
			recovery_key_path = (
				args.recovery_key if args.recovery_key != 'recovery_key.txt' else 'recovery_key.txt'
			)
			recovery_key_data = RecoveryKeyUtils.read_recovery_key(recovery_key_path)
