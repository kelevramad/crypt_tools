        ...
        for i in range(num_shares):
            ...
            for j in range(len(secret)):
                ...
                for deg in range(1, threshold):
                    y ^= GaloisField.mul(
                        coeffs[deg - 1], GaloisField.exp(x, deg))
                share[j + 1] = y
...
class UIHelpers:
    ...
    def render_qr(data: str) -> str:
        ...
        for row_index in range(0, len(matrix), 2):
            ...
            bottom_row = (
                matrix[row_index + 1] if row_index +
                1 < len(matrix) else [False] * len(top_row)
            )
            line = []
    ...
    def file_selector(start_path: str = '.') -> Optional[str]:
        ...
        if not sys.stdin.isatty() or not sys.stdout.isatty():
            raise RuntimeError(
                'Interactive file selection requires an interactive terminal.')

        term = Terminal()
        current_dir = os.path.abspath(
            start_path if os.path.isdir(
                start_path) else os.path.dirname(start_path) or '.'
        )
        selected_index = 0
        scroll_offset = 0
        entries = []
        ...
        def build_entries(directory: str):
            ...
            items = [
                ...
                {'label': '⬆️  [..] Go to parent directory',
                    'path': parent_dir, 'action': 'up'},
            ]
            for name in sorted(
                    os.listdir(directory),
                    key=lambda item: (not os.path.isdir(
                        os.path.join(directory, item)), item.lower()),
            ):
                full_path = os.path.join(directory, name)
                if os.path.isdir(full_path):
                    items.append(
                        {'label': f'📁 {name}', 'path': full_path, 'action': 'enter'})
                else:
                    items.append(
                        {'label': f'📄 {name}', 'path': full_path, 'action': 'select'})
            return items
        ...
        def render_screen():
            ...
            for idx, entry in enumerate(visible_entries, start=scroll_offset):
                ...
                style_fn = term.black_on_cyan if idx == selected_index else (
                    lambda text: text)
...
class HeaderParser:
    ...
    def parse_ct02(data: bytes) -> dict:
        ...
        return {
            ...
            'use_recovery': bool(flags & Config.FLAG_RECOVERY),
            'kdf_id': kdf_id,
    ...
    def parse_hidden_footer(path: str) -> Optional[dict]:
        ...
        min_blob = Config.FIXED_HEADER_SIZE + Config.SALT_SIZE + \
            Config.NONCE_SIZE + Config.TAG_SIZE
        ...
        outer_total_len = int.from_bytes(
            footer[4:12], byteorder='big', signed=False)
    ...
    def inspect_threshold(path: str) -> Optional[dict]:
        ...
        share_counts_offset = metadata['header_len'] + \
            metadata['salt_len'] + metadata['nonce_len']
...
class ConfigParser:
    ...
    def load_defaults(explicit_config_path: Optional[str] = None):
        config_path, explicit = ConfigParser.discover_path(
        ...
        if config_path:
            try:
                ...
                raise ValueError(
                    f'Failed to load config file {config_path}: {e}') from e
        elif explicit:
        ...
        for env_key, normalized_key in ENV_KEY_ALIASES.items():
            ...
            if raw_value in [None, '']:
                continue
            env_defaults[normalized_key] = ConfigParser.coerce_value(
    ...
    def apply_defaults(args, cli_overrides, config_defaults, env_defaults):
        ...
        if 'password' not in cli_overrides and not args.password and 'password' in merged_defaults:
            ...
            args.password = password_value if isinstance(
                password_value, list) else [password_value]
...
def _inspect_ct02_blob_from_path(
    ...
    result = {
        ...
        'recovery_key': 'enabled' if metadata.get('use_recovery', False) else 'disabled',
        'kdf': 'PBKDF2-SHA256'
    ...
    if metadata['flags'] & Config.FLAG_THRESHOLD:
        ...
        share_size = Config.SALT_SIZE + Config.NONCE_SIZE + \
            Config.KEY_SIZE + 1 + Config.TAG_SIZE
...
class KeyFileUtils:
    ...
    def generate(path: str, key_size: int = 16) -> bool:
        ...
        try:
            ...
            recovery_key = base64.urlsafe_b64encode(
                key).decode('ascii').rstrip('=')
            with open(path, 'w', encoding='utf-8', newline='\n') as f:
                f.write(f'{recovery_key}\n')
            ConsoleLogger.show(
                'success', f'Recovery key file generated: {path}')
            ConsoleLogger.show(
                'info', f'Key size: {key_size} bytes ({key_size * 8} bits)')
            return True
        except Exception as e:
    ...
    def read(path: str) -> Optional[bytes]:
        ...
        try:
            ...
            if len(raw_data) > 4096:
                raise ValueError(
                    f'Key file too large: {len(raw_data)} bytes (maximum 4096)')
            ...
            try:
                ...
                if text_data:
                    ...
                    key_data = base64.b64decode(
                        text_data + padding, altchars=b'-_', validate=True)
            ...
            if len(key_data) < 16:
                raise ValueError(
                    f'Key file too small: {len(key_data)} bytes (minimum 16)')

            if len(key_data) > 1024:
                raise ValueError(
                    f'Key file too large: {len(key_data)} bytes (maximum 1024)')

            ConsoleLogger.show(
                'debug', f'Read key file: {path} ({len(key_data)} bytes)')
            return key_data
...
class RecoveryKeyUtils:
    ...
    def generate() -> bytes:
        ...
        return os.urandom(Config.RECOVERY_KEY_SIZE)

    @staticmethod
    def encrypt_derived_key(derived_key: bytes, recovery_key: bytes) -> bytes:
        """
        ...
        cipher = AES.new(recovery_key, AES.MODE_GCM, nonce=nonce)
        ciphertext, tag = cipher.encrypt_and_digest(derived_key)
        return nonce + ciphertext + tag

    @staticmethod
    def decrypt_derived_key(encrypted_blob: bytes, recovery_key: bytes) -> Optional[bytes]:
        """
        Decrypt the recovery blob to retrieve the original derived key.
        Returns the derived key or None if decryption fails.
        """
        if len(encrypted_blob) < Config.NONCE_SIZE + Config.RECOVERY_KEY_SIZE + Config.TAG_SIZE:
            return None
        nonce = encrypted_blob[:Config.NONCE_SIZE]
        ciphertext = encrypted_blob[Config.NONCE_SIZE:-Config.TAG_SIZE]
        tag = encrypted_blob[-Config.TAG_SIZE:]
        try:
            cipher = AES.new(recovery_key, AES.MODE_GCM, nonce=nonce)
    ...
    @staticmethod
    def write_recovery_key(path: str, recovery_key: bytes) -> bool:
        """Write recovery key to file in MEGA-style base64 format."""
        try:
            b64_key = base64.urlsafe_b64encode(
                recovery_key).decode('ascii').rstrip('=')
            with open(path, 'w', encoding='utf-8', newline='\n') as f:
                f.write(f'{b64_key}\n')
            ConsoleLogger.show(
                'success', f'Recovery key file generated: {path}', icon='🔑')
            ConsoleLogger.show(
                'info', f'Key size: {Config.RECOVERY_KEY_SIZE} bytes ({Config.RECOVERY_KEY_SIZE * 8} bits)')
            ConsoleLogger.show(
                'warning', 'Store recovery key securely - it can decrypt your files without a password!', icon='🔐')
            return True
        except Exception as e:
            ConsoleLogger.show('error', f'Failed to write recovery key: {e}')
            return False

    @staticmethod
    def read_recovery_key(path: str) -> Optional[bytes]:
        ...
        try:
            ...
            if len(key_data) != Config.RECOVERY_KEY_SIZE:
                raise ValueError(
                    f'Invalid recovery key size: {len(key_data)} bytes (expected {Config.RECOVERY_KEY_SIZE})')

            ConsoleLogger.show(
                'debug', f'Read recovery key: {path} ({len(key_data)} bytes)')
            return key_data
        except Exception as e:
...
class CryptoEngine:
    ...
    def _derive_key(
        ...
        if kdf_type == Config.KDF_ARGON2:
            ...
            ConsoleLogger.show(
                'debug', f'Deriving key with Argon2id ({iterations} iterations)')

            if keyfile_data:
                ConsoleLogger.show(
                    'debug', 'Using key file for key derivation')
                derived_from = KeyFileUtils.combine_password_and_keyfile(
                    password, keyfile_data)
            else:
        ...
        ConsoleLogger.show(
            'debug', f'Deriving key with PBKDF2 ({iterations} iterations)')
    ...
    def encrypt_data(
            ...
            kdf_type: int = Config.KDF_PBKDF2,
            iterations: int = Config.PBKDF2_ITERATIONS,
            recovery_key_path: Optional[str] = None,
    ) -> bytes:
        """
        Encrypt bytes in memory.
        Format v2.7: [HEADER] + [SALT] + [NONCE] + [RECOVERY_BLOB (optional)] + [CIPHERTEXT] + [TAG]
        Returns the encrypted bytes. If recovery_key_path is provided, also writes the recovery key file.
        """
        ConsoleLogger.show(
            'debug', f'Starting in-memory data encryption ({len(data)} bytes input)')
        salt = os.urandom(Config.SALT_SIZE)
        nonce = os.urandom(Config.NONCE_SIZE)
        use_keyfile = keyfile_data is not None
        use_recovery = recovery_key_path is not None

        header = HeaderParser.build_header(
            is_text=True, use_keyfile=use_keyfile, use_recovery=use_recovery,
            kdf_id=kdf_type, iterations=iterations
        )
        ConsoleLogger.show(
            'debug',
        ...
        key = self._derive_key(
            password, salt, keyfile_data, kdf_type, iterations)

        # Generate and encrypt recovery key if requested
        recovery_blob = None
        if use_recovery:
            ConsoleLogger.show('info', 'Generating recovery key...', icon='🔑')
            recovery_key = RecoveryKeyUtils.generate()
            recovery_blob = RecoveryKeyUtils.encrypt_derived_key(
                key, recovery_key)

            # Write recovery key to file
            if not RecoveryKeyUtils.write_recovery_key(recovery_key_path, recovery_key):
                ConsoleLogger.show(
                    'error', 'Failed to generate recovery key file')
                raise RuntimeError('Failed to write recovery key file')
        ...
        # Build final output
        result = header + salt + nonce
        if recovery_blob:
            # Write length (2 bytes) + blob
            result += len(recovery_blob).to_bytes(2,
                                                  byteorder='big') + recovery_blob
    ...
    def decrypt_data(
            ...
            recovery_key_data: Optional[bytes] = None,
    ) -> Optional[bytes]:
        """
        Decrypt bytes in memory.
        Supports:
        - v2.7: [HEADER] + [SALT] + [NONCE] + [RECOVERY_BLOB (optional)] + [CIPHERTEXT] + [TAG]
        - v2.2: [HEADER] + [SALT] + [NONCE] + [CIPHERTEXT] + [TAG]
        - legacy text payload: [SALT] + [NONCE] + [TAG] + [CIPHERTEXT]
        Can decrypt with either password OR recovery key.
        """
        try:
            ConsoleLogger.show(
                'debug',
            ...
            if metadata['is_legacy']:
                ...
                nonce = enc_data[
                    metadata['salt_len']: metadata['salt_len'] +
                    metadata['nonce_len']
                ]
                ...
                use_recovery = metadata.get('use_recovery', False)
                recovery_blob = None
                ciphertext_offset = 0

                if use_recovery:
                    # Read recovery blob length (2 bytes)
                    recovery_len_pos = nonce_end
                    recovery_len = int.from_bytes(
                        enc_data[recovery_len_pos:recovery_len_pos +
                                 2], byteorder='big'
                    )
                    recovery_blob = enc_data[
                        recovery_len_pos + 2: recovery_len_pos + 2 + recovery_len
                    ]
                    ciphertext_offset = 2 + recovery_len
                    ConsoleLogger.show(
                        'debug', f'Read recovery blob ({recovery_len} bytes)')

                ciphertext = enc_data[nonce_end +
                                      ciphertext_offset: -metadata['tag_len']]
            ...
            key = None
            if recovery_key_data and use_recovery and recovery_blob:
                ConsoleLogger.show(
                    'info', 'Attempting decryption with recovery key...', icon='🔑')
                key = RecoveryKeyUtils.decrypt_derived_key(
                    recovery_blob, recovery_key_data)
                if key:
                    ConsoleLogger.show(
                        'debug', 'Successfully recovered encryption key from recovery key')
                else:
                    ConsoleLogger.show(
                        'warning', 'Recovery key decryption failed, falling back to password')
            ...
            ConsoleLogger.show(
                'debug', 'Initializing AES-GCM cipher for decryption')
            cipher = AES.new(key, AES.MODE_GCM, nonce=nonce)

            ConsoleLogger.show(
                'debug', 'Verifying tag and decrypting ciphertext')
    ...
    def encrypt_file(
        ...
        try:
            ConsoleLogger.show(
                'debug', f'Starting file encryption: {input_path} -> {output_path}')
            file_size = os.path.getsize(input_path)
            ...
            if use_recovery:
                ConsoleLogger.show(
                    'info', 'Generating recovery key...', icon='🔑')
                recovery_key = RecoveryKeyUtils.generate()
                recovery_blob = RecoveryKeyUtils.encrypt_derived_key(
                    key, recovery_key)
            ...
            with open(input_path, 'rb') as fin, open(output_path, 'wb') as fout:
                ...
                if recovery_blob:
                    ConsoleLogger.show(
                        'debug', f'Writing recovery blob ({len(recovery_blob)} bytes)')
                    # Write length (2 bytes big-endian) + blob
                    fout.write(len(recovery_blob).to_bytes(2, byteorder='big'))
                    fout.write(recovery_blob)

                compressor = zlib.compressobj(level=9) if compress else None
                if compress:
                    ConsoleLogger.show(
                        'debug', 'Compression enabled (zlib level 9)')
                ...
                tag = cipher.digest()
                ConsoleLogger.show(
                    'debug', f'Writing authentication tag ({len(tag)} bytes)')
                fout.write(tag)
    ...
    def decrypt_file(
        ...
        try:
            ...
            with open(input_path, 'rb') as fin:
                ...
                prefix = fin.read(Config.FIXED_HEADER_SIZE)
                metadata = HeaderParser.parse_format(
                    prefix, text_payload=False)
                if metadata['is_legacy']:
                    if slice_start != 0:
                        raise ValueError(
                            'Legacy format does not support container slices')
                    fin.seek(slice_start)
                ...
                if use_recovery:
                    # Read recovery blob length (2 bytes)
                    recovery_len_bytes = fin.read(2)
                    if len(recovery_len_bytes) < 2:
                        raise ValueError('Invalid recovery blob length')
                    recovery_len = int.from_bytes(
                        recovery_len_bytes, byteorder='big')
                    recovery_blob = fin.read(recovery_len)
                    ciphertext_offset = 2 + recovery_len
                    ConsoleLogger.show(
                        'debug', f'Read recovery blob ({recovery_len} bytes)')
                ...
                with (
                    ...
                    while bytes_read < ciphertext_len:
                        read_size = min(Config.CHUNK_SIZE,
                ...
                if bytes_read < ciphertext_len:
                    ConsoleLogger.show(
                        'warning', 'Unexpected end of file while reading ciphertext')

                    if decompressor:
                        ConsoleLogger.show(
                            'debug', 'Flushing decompressor buffers')
                        fout.write(decompressor.flush())

                # Verify Tag (always, regardless of bytes_read)
                tag = fin.read(metadata['tag_len'])
                ConsoleLogger.show(
                    'debug', f'Read authentication tag ({len(tag)} bytes)')
                try:
                    ...
                    ConsoleLogger.show(
                        'error', f'Decryption failed for: {input_path}')
                    fout.close()
                    os.remove(output_path)
                    return False

            ConsoleLogger.show(
                'success', 'Integrity Verified. Decryption successful.')
            return True
    ...
    def encrypt_hidden_container(
        ...
        tmp_hidden = None
        try:
            fd_o, tmp_outer = tempfile.mkstemp(
                prefix='ct_outer_', suffix='.enc')
            os.close(fd_o)
            fd_h, tmp_hidden = tempfile.mkstemp(
                prefix='ct_hidden_', suffix='.enc')
            os.close(fd_h)
            ...
            ConsoleLogger.show(
                'error', f'Hidden container encryption error: {e}')
            if os.path.exists(output_path):
                try:
                    os.remove(output_path)
                except OSError:
    ...
    def inspect_file(self, input_path: str) -> dict:
        ...
        if not os.path.exists(input_path) or not os.path.isfile(input_path):
            raise FileNotFoundError(
                f"ENOENT: no such file or directory, stat '{input_path}'")
        ...
        result = _inspect_ct02_blob_from_path(
            input_path, blob_start=0, blob_span=outer_span)
    ...
    def encrypt_with_threshold(
        ...
        try:
            ...
            ConsoleLogger.show(
                'debug', 'Generated master key for threshold encryption')

            shares = ShamirSecretSharing.generate_shares(
                master_key, num_passwords, threshold)
            ConsoleLogger.show(
                'debug',
            ...
            nonce = os.urandom(Config.NONCE_SIZE)
            key = self._derive_key(
                'threshold-dummy', salt, None, kdf_type, iterations)
            ...
            with open(input_path, 'rb') as fin, open(output_path, 'wb') as fout:
                ...
                for i in range(num_passwords):
                    ...
                    encrypted_share = self._encrypt_share_with_password(
                        shares[i], password_key)
    ...
    def decrypt_with_threshold(
        ...
        try:
            ...
            file_size = end - slice_start
            ConsoleLogger.show(
                'debug', f'Starting threshold file decryption: {input_path}')
            ...
            with open(input_path, 'rb') as fin:
                ...
                if not (metadata['flags'] & Config.FLAG_THRESHOLD):
                    raise ValueError(
                        'File is not encrypted with threshold mode')
                ...
                for _ in range(num_passwords):
                    ...
                    encrypted_shares.append(
                        {'salt': password_salt, 'data': encrypted_share})
    ...
    def _try_reconstruct_master_key(
        ...
        for pw in passwords:
            ...
            for idx, enc_share in enumerate(encrypted_shares):
                ...
                try:
                    ...
                    share = self._decrypt_share_with_password(
                        enc_share['data'], password_key)
                    ConsoleLogger.show(
                        'debug', f'  Share {idx} decrypted successfully!')
                    share_id = share[0]
        ...
        try:
            master_key = ShamirSecretSharing.recover_secret(
                share_data_list[:threshold])
            ConsoleLogger.show(
                'debug', 'Successfully reconstructed master key')
            return master_key
        except Exception as e:
            ConsoleLogger.show(
                'error', f'Failed to reconstruct master key: {e}')
            return None
...
class PasswordStrength:
    ...
    STRENGTHS = {
        ...
        # Red
        'WEAK': {'label': 'Weak', 'color': '\033[91m', 'reset': '\033[0m', 'icon': '⚠️'},
        'MEDIUM': {
            'label': 'Medium',
            'color': '\033[93m',
            'reset': '\033[0m',
            ...
            while True:
                ...
                if char == '\r' or char == '\n':
                    ...
                    if password:
                        ...
                        strength_indicator = PasswordStrength.get_indicator(
                            password)
                ...
                while True:
                    ...
                    if char == '\r' or char == '\n':
                        ...
                        if password:
                            ...
                            strength_indicator = PasswordStrength.get_indicator(
                                password)
                            char_types = PasswordStrength.get_char_types(
                                password)
                            asterisks = '*' * len(password)
                            sys.stdout.write(
                                f'\r{white}[{reset}🔑{white}]{reset} {prompt}{asterisks}  {strength_indicator}  {char_types}\033[K'
    ...
    def verify_with_strength(
        ...
        if not sys.stdin.isatty():
            ...
            if not password:
                ...
                ConsoleLogger.show(
                    'error', 'Operation aborted: No password provided')
                sys.exit(1)
            password2 = getpass.getpass(prompt2)
            if password != password2:
                ConsoleLogger.show('error', 'Passwords do not match!')
                ConsoleLogger.show(
                    'error', 'Operation aborted due to password mismatch')
                sys.exit(1)
            return password
...
def parse_args(argv=None):
    ...
    utility_group.add_argument(
        '--log', action='store_true', help='Enable logging to file')
    utility_group.add_argument(
        '-v', '--version', action='version', version=Config.VERSION)
    utility_group.add_argument(
    ...
    mode_group.add_argument(
        '-e', '--encrypt', action='store_true', help='Encrypt mode (default)')
    mode_group.add_argument(
        '-d', '--decrypt', action='store_true', help='Decrypt mode')
    mode_group.add_argument(
        '--inspect', action='store_true', help='Inspect encrypted file metadata'
    )

    input_group = parser.add_argument_group(
        UIHelpers.heading('📥', 'Input & Output'))
    ...
    secret_group = parser.add_argument_group(
        UIHelpers.heading('🔑', 'Passwords & Secrets'))
    ...
    secret_group.add_argument(
        '--recovery-key',
        nargs='?',
        const='recovery_key.txt',
        default=None,
        help='Generate/use recovery key file (default: recovery_key.txt); encrypt generates key, decrypt uses it',
    )

    file_group = parser.add_argument_group(
        UIHelpers.heading('📦', 'File & Container Behavior'))
    ...
    crypto_group = parser.add_argument_group(
        UIHelpers.heading('🧬', 'Crypto Tuning'))
...
def _log_completion_summary(
    ...
    if total_ops > 0 and success_count == total_ops:
        ConsoleLogger.show(
            'success', f'{action} completed successfully', icon='✅')
    elif success_count == 0:
        pass
    else:
        ConsoleLogger.show(
            'warning', f'{action} completed with failures', icon='⚠️')
...
def main(argv=None):
    ...
    try:
        config_path, config_defaults, env_defaults = ConfigParser.load_defaults(
            args.config)
    except ValueError as e:
        ConsoleLogger.show('error', str(e))
        sys.exit(1)
    args = ConfigParser.apply_defaults(
        args, cli_overrides, config_defaults, env_defaults)
    engine = CryptoEngine()
    ...
    if args.qr and (args.decrypt or not args.text):
        ConsoleLogger.show(
            'error', '--qr is only supported with text encryption')
        sys.exit(1)
    if args.select:
        try:
    ...
    if args.hidden_vol and args.text:
        ConsoleLogger.show(
            'error', '--hidden-vol applies only to file encryption')
        sys.exit(1)
    ...
    if args.hidden_vol and args.inspect:
        ConsoleLogger.show(
            'error', '--hidden-vol cannot be used with --inspect')
        sys.exit(1)
    ...
    if args.threshold:
        ...
        if args.text:
            ConsoleLogger.show(
                'error', '--threshold applies only to file encryption/decryption')
            sys.exit(1)
        if args.hidden_vol:
            ConsoleLogger.show(
                'error', '--threshold cannot be used with --hidden-vol')
            sys.exit(1)
    ...
    ConsoleLogger.show(
        'info', f'Session started at {start_timestamp}', icon='🕐')
    if config_path:
    ...
    def finish_session():
        ...
        ConsoleLogger.show(
            'info', f'Session ended at {end_timestamp}', icon='🏁')
        if ConsoleLogger.LOG_ENABLED:
            ConsoleLogger.show(
                'info', '=' * 80, show_console=False, log_file=True)
    ...
    if args.debug:
        ...
        ConsoleLogger.show(
            'debug', 'Debug Mode Enabled. Verbose logging activated.')
        ConsoleLogger.show('info', 'Debug mode: Enabled')

    # Show log file info (after LOG_ENABLED is set)
    if args.log:
        ConsoleLogger.show(
            'debug', f'Logging enabled. Writing to: {ConsoleLogger.LOG_FILE}')
    ...
    if args.file:
        ...
        if has_wildcard:
            ...
            if not file_list:
                ConsoleLogger.show(
                    'error', f'No files matched pattern: {args.file}')
                ConsoleLogger.show(
                    'error', 'Operation failed: No matching files')
                abort(1)
        else:
            file_list = [args.file]
    ...
    if args.hidden_vol:
        if not args.file:
            ConsoleLogger.show(
                'error', '--hidden-vol requires -f/--file (decoy path)')
            abort(1)
        if args.recursive:
            ConsoleLogger.show(
                'error', '--hidden-vol cannot be used with --recursive')
            abort(1)
        ...
        if not os.path.isfile(decoy_p):
            ConsoleLogger.show(
                'error', 'Decoy path must be a regular file for --hidden-vol')
            abort(1)
        if not os.path.isfile(args.hidden_file):
            ConsoleLogger.show(
                'error', f'Hidden file not found: {args.hidden_file}')
            abort(1)
    ...
    if args.inspect:
        ...
        try:
            ...
            ConsoleLogger.show(
                'error', 'Operation failed: File does not exist')
            abort(1)
        except ValueError as e:
            ConsoleLogger.show('error', f'Inspect failed: {e}')
            ConsoleLogger.show(
                'error', f'File is not a supported encrypted file: {target_file}')
            abort(1)
        except Exception as e:
            ConsoleLogger.show('error', f'Inspect failed: {e}')
            ConsoleLogger.show(
        ...
        ConsoleLogger.show(
            'info', f'Legacy: {"yes" if details["legacy"] else "no"}', icon='🕰️')
        ConsoleLogger.show(
            'info', f'Compression: {details["compression"]}', icon='🗜️')
        ConsoleLogger.show(
            'info', f'Keyfile: {details.get("keyfile", "disabled")}', icon='🔑')
        ConsoleLogger.show(
            'info', f'Recovery key: {details.get("recovery_key", "disabled")}', icon='🔐')
        ConsoleLogger.show('info', f'KDF: {details["kdf"]}', icon='🧬')
        ConsoleLogger.show(
            'info', f'Iterations: {details["iterations"]}', icon='🔁')
        ConsoleLogger.show(
            'info', f'Threshold mode: {details["thresholdMode"]}', icon='🧩')
        if details.get('numPasswords') is not None:
            ConsoleLogger.show(
                'info', f'Shares: {details["numPasswords"]}', icon='🔢')
            ConsoleLogger.show(
                'info',
        ...
        ConsoleLogger.show(
            'info', f'Salt length: {details["saltLength"]}', icon='🧂')
        ConsoleLogger.show(
            'info', f'Nonce length: {details["nonceLength"]}', icon='🎲')
        ConsoleLogger.show(
            'info', f'Tag length: {details["tagLength"]}', icon='🏷️')
        ConsoleLogger.show(
            'info', f'Header length: {details["headerLength"]}', icon='🧱')
        ConsoleLogger.show(
            'info', f'File size: {details["fileSize"]} bytes', icon='📦')
        ConsoleLogger.show(
            'info', f'Ciphertext size: {details["ciphertextSize"]} bytes', icon='🔐')
        if details.get('container') == 'hidden':
            ...
            if inner:
                ...
                ConsoleLogger.show(
                    'info', f'Inner compression: {inner["compression"]}', icon='🗜️')
                ConsoleLogger.show(
                    'info', f'Inner keyfile: {inner["keyfile"]}', icon='🔑')
                ConsoleLogger.show(
                    'info', f'Inner KDF: {inner["kdf"]}', icon='🧬')
                ConsoleLogger.show(
                    'info', f'Inner iterations: {inner["iterations"]}', icon='🔁')
                ConsoleLogger.show(
                    'info',
                ...
                if inner.get('numPasswords') is not None:
                    ConsoleLogger.show(
                        'info', f'Inner shares: {inner["numPasswords"]}', icon='🔢')
                    ConsoleLogger.show(
                        'info',
        ...
        try:
            ...
            ConsoleLogger.show(
                'info', f'Format: {details["format"]}', icon='📋')
            ConsoleLogger.show(
                'info', f'Version: {details["version"]}', icon='📋')
            ConsoleLogger.show(
                'info', f'Compression: {details["compression"]}', icon='📋')
            ConsoleLogger.show('info', f'KDF: {details["kdf"]}', icon='🧬')
            ConsoleLogger.show(
                'info', f'Iterations: {details["iterations"]}', icon='📋')
            ConsoleLogger.show(
                'info', f'Header Length: {details["headerLength"]} bytes', icon='📋')
            ConsoleLogger.show(
            ...
            if 'supported encrypted file' in str(e):
                ConsoleLogger.show(
                    'error', 'Only CT02 supported encrypted file')
            ConsoleLogger.show(
                'error', 'Operation failed: Cannot inspect file')
            abort(1)
    ...
    if args.text:
        ...
        ConsoleLogger.show(
            'info', f'Mode: {mode_str}', icon='🔐' if not args.decrypt else '🔓')
        ...
        if (
            ...
            try:
                ...
                ConsoleLogger.show(
                    'debug', f'Could not inspect compression metadata: {e}')
        ConsoleLogger.show(
            'info', f'Mode: {mode_str}', icon='🔐' if not args.decrypt else '🔓')
        ConsoleLogger.show('info', f'Compression: {compression_str}', icon='📦')
        ...
        if is_dir and args.recursive:
            ConsoleLogger.show(
                'info', f'Processing directory: {args.file}', icon='📁')
            ConsoleLogger.show(
                'info',
            ...
            if file_list and len(file_list) > 1:
                ConsoleLogger.show(
                    'info', f'Processing files: {len(file_list)}', icon='📄')
            else:
    ...
    if args.keyfile:
        ...
        if keyfile_data is None:
            ConsoleLogger.show(
                'error', 'Operation aborted: Could not load key file')
            abort(1)
    ...
    recovery_key_data = None
    if args.recovery_key:
        if args.decrypt:
            # In decrypt mode, load the recovery key file
            recovery_key_path = args.recovery_key if args.recovery_key != 'recovery_key.txt' else 'recovery_key.txt'
            recovery_key_data = RecoveryKeyUtils.read_recovery_key(
                recovery_key_path)
            if recovery_key_data is None:
                ConsoleLogger.show(
                    'error', 'Operation aborted: Could not load recovery key file')
                abort(1)
            ...
            recovery_key_path = args.recovery_key if args.recovery_key != 'recovery_key.txt' else 'recovery_key.txt'
            ConsoleLogger.show(
                'info', f'Recovery key will be saved to: {recovery_key_path}', icon='🔑')

    # Validate recovery key usage
    if args.recovery_key and args.hidden_vol:
        ConsoleLogger.show(
            'error', '--recovery-key cannot be used with --hidden-vol')
        abort(1)

    args.kdf = args.kdf or 'pbkdf2'
    if args.kdf not in ['pbkdf2', 'argon2']:
        ConsoleLogger.show(
            'error', 'Invalid --kdf value. Must be "pbkdf2" or "argon2"')
        abort(1)
    ...
    ConsoleLogger.show(
        'important', f'KDF: {args.kdf} ({iterations} iterations)', icon='🧬')
    ...
    if (
        ...
        try:
            threshold_requirements = HeaderParser.inspect_threshold(
                path=args.file)
        except Exception as e:
            ConsoleLogger.show(
                'debug', f'Could not inspect threshold requirements: {e}')
    ...
    if args.hidden_vol:
        if pw_outer is None:
            ...
            ConsoleLogger.show(
                'debug', 'Decoy password provided via command line')
        if pw_hidden is None:
            ...
            ConsoleLogger.show(
                'info', 'Hidden volume password entered', icon='🔑')
        else:
            ConsoleLogger.show(
                'debug', 'Hidden password provided via command line')
        ...
        if len(args.password) < num_passwords_needed:
            ConsoleLogger.show(
                'info', f'Threshold mode: need {num_passwords_needed} passwords')
            for i in range(len(args.password), num_passwords_needed):
                pw = PasswordUtils.prompt_with_strength(
                    f'Enter password {i + 1}/{num_passwords_needed}: '
                )
        ...
        if args.password and len(args.password) == 1:
            ...
            if args.decrypt and args.hidden and args.password_hidden is not None:
                ...
                ConsoleLogger.show(
                    'debug', 'Using --password-hidden for inner decrypt')
            elif not args.decrypt:
                args.password = PasswordUtils.verify_with_strength()
                ConsoleLogger.show(
                    'info', 'Password verification entered', icon='🔄')
            else:
                args.password = PasswordUtils.prompt_with_strength()
                ConsoleLogger.show(
                    'info', 'Password entered by user', icon='🔑')
    ...
    if args.text:
        ...
        if not args.decrypt:
            ...
            result = engine.encrypt_data(
                args.text.encode(
                    'utf-8'), args.password, keyfile_data, kdf_type, iterations,
            ...
            ConsoleLogger.show(
                'success', 'Encryption completed successfully', icon='✅')
            ConsoleLogger.show('info', 'Operations completed: 1/1', icon='✔️')
            ConsoleLogger.show(
                'info', f'Total time: {elapsed_time:.2f}s', icon='⏱️')
        else:
            ...
            result = engine.decrypt_data(
                raw_data, args.password, keyfile_data, recovery_key_data)
            if result:
                ConsoleLogger.show(
                ...
                ConsoleLogger.show(
                    'success', 'Decryption completed successfully', icon='✅')
                ConsoleLogger.show(
                    'info', 'Operations completed: 1/1', icon='✔️')
                ConsoleLogger.show(
                    'info', f'Total time: {elapsed_time:.2f}s', icon='⏱️')
        ...
        if args.recursive and os.path.isdir(args.file):
            ...
            for root, dirs, files in os.walk(input_dir):
                for file in files:
                    ...
                    if not args.decrypt:
                        ...
                        ConsoleLogger.show(
                            'info', f'Processing: {file_path}', icon='📄')
                        ...
                        if foot:
                            ...
                            ok_batch = engine.decrypt_file(
                                ...
                                recovery_key_data=recovery_key_data
                            )
                        if ok_batch:
                            success_count += 1
                            ConsoleLogger.show(
            ...
            if fail_count > 0:
                ConsoleLogger.show(
                    'warning', f'Some files failed to process: {fail_count} failed')
            ...
            for target in targets:
                ...
                if not args.decrypt:
                    if args.hidden_vol:
                        ...
                        single_password = PasswordUtils.normalize(
                            args.password)
                        ...
                        try:
                            with open(target, 'rb') as f:
                                ...
                                if header[:4] == Config.MAGIC:
                                    flags = header[5]
                                    is_threshold_file = bool(
                                        flags & Config.FLAG_THRESHOLD)
                        except Exception:
                            pass
            ...
            _log_completion_summary(
                args.decrypt, success_count, total_ops, elapsed_time)
